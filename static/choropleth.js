...
Promise.all([
  ...
  .then(([world, data]) => {
    ...
    const projection = d3.geoNaturalEarth1().fitSize([width, height], world); // Assuming geojsonData contains your features
    const pathGenerator = d3.geoPath().projection(projection);
    const dataMap = new Map(data.map((item) => [item.country, item.count]));
    const colorScale = d3
      .scaleSequential((t) => d3.interpolateReds(t * 0.8 + 0.2))
