...
function WordCloud2() {
  d3.json(`http://127.0.0.1:5000/word_cloud_data`)
    .then(function (apiData) {
      console.log("inside word cloud 2");
      console.log("word cloud ", apiData);
      // var text = JSON.parse(apiData).map(function(item) {
      ...
      (size = (group) => group.length), // Given a grouping of words, returns the size factor for that word
        ...
        (padding = 0), // amount of padding between the words (in pixels)
        (rotate = 0), // a constant or function to rotate the words
        (invalidation = false); // when this promise resolves, stop the simulation

      var words = ["happy", "join", "today", "go", "history"];
